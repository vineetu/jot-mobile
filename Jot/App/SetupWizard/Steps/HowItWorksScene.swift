//
//  HowItWorksScene.swift
//  Jot
//
//  Self-contained, reusable "how the capture loop works" illustration —
//  extracted from `HowItWorksStep` (wizard W4) so BOTH the wizard AND the
//  Help → "How Jot works" page can consume one source of truth.
//
//  The bundle is everything the teaching moment needs, with no wizard-state
//  coupling:
//    • the animated mini-phone `HowScene` (a pure function of `(phase, step)`),
//    • the four numbered step rows that highlight in lockstep,
//    • the verbatim honest "we'd skip the hop if we could" footnote,
//    • a self-looping `TimelineView(.animation)` driver (anchored on appear),
//    • the Reduce-Motion static-frame branch.
//
//  The wizard W4 must look/behave IDENTICALLY after this extraction — the view
//  bodies below are lifted verbatim from the old `HowItWorksStep`.
//

import SwiftUI

// MARK: - Reusable bundle (scene + steps + footnote + driver)

/// Drop-in teaching block: animated mini-phone + numbered step list + the
/// honest footnote, with its own self-looping clock. Used by wizard W4 and the
/// Help "How Jot works" page.
///
/// Pass `showFootnote: false` to render only the scene + step list (the wizard
/// supplies the footnote itself in its footer slot, above the Got it button).
struct HowItWorksScene: View {
    /// When `true`, render the honest footnote below the step list. The wizard
    /// keeps it in its own footer, so it passes `false`.
    ...
    /// The verbatim honest note. Exposed so the wizard can render it in its
    /// footer slot from the same source of truth.
    static let honestFootnote = "We'd skip this step if we could. Apple doesn't let keyboards use the mic directly — so Jot hops back to capture. If that ever changes, this goes away."
    ...
    /// holds for its full segment and the scene + list stay in lockstep.
    @ViewBuilder
    ...
    /// 1…4 for the four segments of the loop.
...
// MARK: - HowScene (animated mini-phone, 168×248, 13s looping, 4 step cues)

/// Mini-phone illustration teaching the four-step capture loop. `phase` (0→1
/// over the loop) and `step` (1…4, or 0 in the reduce-motion static frame) come
/// from the parent's `TimelineView` so the scene and the step list never drift.
...
struct HowScene: View {
    ...
    /// Progress within the current step segment, 0→1.
