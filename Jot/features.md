...
- **The accurate transcript comes after you stop.** The live preview is a rough, fast pass over the recent audio; the same model then re-reads the whole recording on stop to finalize punctuation and fix the rough text ([§2.8](#2-8-live-preview-and-final-transcript)). The live text is a hint, not the product — so it's shown small, italic, and fading, never the focus.
...
- [2. Recording Experience](#2-recording-experience)
  ...
  - [2.8 Live Preview and Final Transcript](#2-8-live-preview-and-final-transcript)
...
### 2.8 Live Preview and Final Transcript
The [live preview](#2-3-live-streaming-transcript) and the final saved transcript come from the **same** accurate on-device model, run in two passes. While the user speaks, the model re-transcribes a trailing window of the most recent audio to produce the rough, fast live preview; when the recording stops, the model transcribes the **whole** recording for the accurate saved version (which may differ from, and corrects, the live preview). Users see the result of the final pass in their library; the preview is only a real-time hint.
...
Jot picks its on-device speech model **automatically, based on the iPhone** — there is no model selector or variant picker. On newer, higher-memory iPhones (iPhone 12 Pro / 13 Pro and iPhone 14 and later) Jot uses the larger, more accurate **"Parakeet 600M"** model, which ships bundled inside the app, so it is available immediately on install with no download and the first dictation works offline. On older or lower-memory iPhones that can't run the larger model (such as iPhone 11, the iPhone 12 and 13 non-Pro, and iPhone SE), Jot automatically uses a smaller English model — **"Parakeet 110M"** — which is **not bundled** (to keep the app download small) and is **fetched once, on the first dictation**; until then the Settings chip reads "Not downloaded" and the location line reads "Downloads on first use · about 220 MB", and during that first fetch the Settings chip shows live download progress. Once downloaded it stays on the iPhone and works offline like the bundled model. The Settings screen surfaces whichever model is active (its name, where it lives, and a readiness chip). There is still no manual download/switch/purge control — the choice is made by the device, and the larger model can't be deleted (it lives in the read-only app bundle and is restored on reinstall). The Speech section shows a non-interactive **Language** row reading "English" (English is the only language today). A "Live text while dictating" toggle in the same section controls whether the [live transcript](#2-3-live-streaming-transcript) is shown during recording. [Vocabulary Boost](#8-vocabulary-boost) always applies, on either model.
...
On newer iPhones the Parakeet 600M speech model ships bundled inside the app (see [§6.1 Speech Model Management](#6-1-speech-model-management)), so there is no speech-model download and this failure mode does not apply. On older/lower-memory iPhones that fetch the smaller Parakeet 110M model on first dictation ([§6.1](#6-1-speech-model-management)), a failed or interrupted fetch surfaces in Settings and on the dictation surface; retrying the dictation re-attempts the download. (AI Rewrite and Ask models are also downloaded — see [§12.3](#12-3-ai-model-download-failure).)
...
The AI Rewrite model re-downloads on first use after a restore; the bundled speech model is present immediately. The Settings → About card surfaces a static "Backed up with iCloud (when iCloud Backup is enabled in iOS Settings)" row so users can confirm this expectation at a glance.
