...
- [4. Setup Wizard](#4-setup-wizard)
  ...
  - [4.5 In-App Try-It (W5)](#4-5-in-app-try-it-w5)
  - [4.6 Keyboard Try-It (W6)](#4-6-keyboard-try-it-w6)
  - [4.7 Warm Hold Opt-In (W7)](#4-7-warm-hold-opt-in-w7)
  - [4.8 Completion (W8)](#4-8-completion-w8)
  - [4.9 Vocabulary Seed (Optional Step 1)](#4-9-vocabulary-seed-optional-step-1)
  - [4.10 AI Rewrite Download Offer (Optional Step 2)](#4-10-ai-rewrite-download-offer-optional-step-2)
  - [4.11 Wizard Navigation Chrome](#4-11-wizard-navigation-chrome)
  - [4.12 Wizard Progress Dots](#4-12-wizard-progress-dots)
- [5. Jot Keyboard](#5-jot-keyboard)
  ...
  - [5.14 Full Access Prompt Screen](#5-14-full-access-prompt-screen)
...
Each transcript row supports a swipe-to-delete gesture and a long-press context menu offering Copy and Delete. Copying a transcript places its text on the clipboard and briefly shows a checkmark confirmation that resets automatically after about a second, confirming the action without an extra tap. Delete triggers a confirmation dialog before permanently removing the entry.
...
The setup wizard has eight core panels (W1–W8) and two optional follow-on panels. The default on-device speech models ship inside the app — see [§6.1 Speech Model Management](#6-1-speech-model-management) — so the wizard no longer needs a "Download speech model" step; dictation works immediately after install with no first-launch download.
...
The third panel sends users to Keyboard Settings once to add Jot and turn on Full Access. When the keyboard has not been detected, the primary call-to-action reads "Open Keyboard Settings"; when the user returns with Jot installed, it changes to "Continue." An always-visible "I've already done this" secondary action advances immediately for users who already completed setup or need to bypass detection. Jot can detect keyboard installation via the system's installed-keyboards list, but it cannot detect Full Access directly; see [§13.3](#13-3-full-access-disclosure).

### 4.4 How It Works (W4)
The fourth panel is a visual explainer illustrating the core usage loop: tap Dictate in the keyboard → record → swipe back to the host app → [cleaned text](#7-1-automatic-cleanup) is pasted automatically. This orientation screen has no required user action other than reading and tapping "Got it".

### 4.5 In-App Try-It (W5)
The fifth panel lets users perform a real live dictation inside the wizard before committing to daily use. W5 records and transcribes inline within the wizard step — the result is shown directly in the step but is not saved to the transcript library (intentional, to avoid polluting transcript history during setup). After recording, the user sees the result and can choose "Sounds good" to continue, "Try again" to record again, or "Skip this step" if something went wrong. A footnote explains the two-model system in plain language.

### 4.6 Keyboard Try-It (W6)
The sixth panel provides a sample text field and instructs the user to switch to the Jot keyboard and dictate something. The panel waits for dictation to complete from the keyboard and advances automatically when one arrives; the inserted result follows the normal [automatic cleanup](#7-1-automatic-cleanup) path. A manual "I tried it" button also allows skipping.

### 4.7 Warm Hold Opt-In (W7)
The seventh panel offers the [Warm Hold](#13-2-warm-hold) feature: keeping the microphone session alive for 60 seconds after each recording to reduce startup latency for the next dictation. The user chooses "Keep mic ready" or "No thanks"; the choice is saved to Settings and can be changed later via [Settings → Privacy](#6-4-privacy-controls), where the duration is also customizable (60s, 2 min, 3 min, or 5 min).

### 4.8 Completion (W8)
The eighth panel congratulates the user with a "You're ready." headline and pitches the two optional wizard steps. Users can proceed to the optional steps immediately ("Set up now") or dismiss the wizard ("Maybe later") and explore those features on their own schedule.

### 4.9 Vocabulary Seed (Optional Step 1)
...
### 4.12 Wizard Progress Dots
At the top of every wizard panel, a row of eight dots represents the eight core setup steps (W1–W8). The current step's dot is larger and filled in the accent color; completed steps are smaller filled dots; upcoming steps are smaller outlined dots. During the optional steps (Vocabulary Seed and AI Rewrite Download), this row is replaced by a different indicator: eight muted mini-dots representing the completed core steps, a short dash separator, and two accent dots tracking progress through the two optional steps. The current optional step's dot follows the same active/upcoming pattern as the core row.
...
- **Dictate**: when Full Access is absent, the Dictate button is replaced by an "Enable Full Access" CTA (with a lock-shield icon) in both the standard and collapsed keyboard views. Tapping it brings the main app to the foreground and presents the [Full Access prompt screen](#5-14-full-access-prompt-screen), which explains why Full Access is required and offers a primary "Open Settings" button.
...
### 5.14 Full Access Prompt Screen
When the keyboard's locked-state "Enable Full Access" pill is tapped (see [§5.11 Full Access Requirement](#5-11-full-access-requirement)), the main app foregrounds and presents a short prompt screen rather than silently jumping to iOS Settings. The screen shows a "Allow Full Access" headline, a one-paragraph explanation that the keyboard pastes dictations into other apps and Full Access is what iOS requires for that, a literal Settings-path breadcrumb (`Settings → General → Keyboard → Keyboards → Jot Keyboard → Allow Full Access`), a primary blue "Open Settings" button that deep-links into iOS Settings at Jot's settings page, and a "Not now" secondary action that dismisses the prompt. The prompt is only reachable by tapping the keyboard's lock pill — opening the deep link from outside Jot will still surface it, but no other flow inside the app navigates to it. See also [§13.3 Full Access Disclosure](#13-3-full-access-disclosure).
...
A Privacy section shows two informational items — that all processing happens on-device and a Full Access row (labeled "Required for paste") — plus a toggle for [Warm Hold](#13-2-warm-hold) after [wizard step W7](#4-7-warm-hold-opt-in-w7). When Warm Hold is on, the toggle is followed by a "Ready for" duration picker with 60s, 2 min, 3 min, and 5 min options. This is the persistent location for the warm-hold setting after the wizard is complete. The Full Access row always displays an "Enabled" status chip; see [§13.3](#13-3-full-access-disclosure) for the disclosure on why this is a static label.

### 6.5 About & Support
An About section provides links to Help & Support (opens [Help](#9-help--onboarding-reference)), Re-run setup wizard (restarts the [Setup Wizard](#4-setup-wizard) from the beginning for troubleshooting or exploration), Send feedback (opens the iOS Mail app with a pre-populated recipient address — tapping this leaves Jot), the app version number, Donations (opens the in-app [Donations](#6-7-donations) screen), a Privacy Policy link, and an Acknowledgements screen (see [§6.6](#6-6-acknowledgements)). When at least one dictation has been recorded, a "Time saved" row appears as the first entry of the About card, showing minutes saved today, the cumulative dictation count, and a small 14-day blue sparkline — the same lifetime stats that the home page used to surface inline, relocated here so the home view stays focused on recent activity (see [§1.1](#1-1-editorial-header)).
...
Jot includes an opt-in cleanup pass that removes filler words ("um", "uh"), discourse-marker phrases ("you know", "I mean"), and short false starts from each dictation without changing the meaning. This is a lexical cleanup pass, not an AI rewrite — the cleaned text appears as the user's Original transcript, not in the Rewrite tab (the Rewrite tab is reserved for [AI Rewrite](#7-2-on-device-ai-rewrite-model) output). The cleanup runs on a small bundled on-device model (about 3.6 MB, trained on the DisfluencySpeech corpus) — no separate download, no Apple Intelligence dependency, and nothing leaves the device. It is off by default; users opt in from [Settings → Speech Model](#6-1-speech-model-management) via the "Clean up filler words" toggle. When enabled, cleanup runs automatically after every dictation across the recording screen and keyboard, and the cleaned text is what gets pasted, copied, and shown as the transcript body in the Original surface. The [Shortcuts file action](#10-1-shortcuts-transcribe-audio-with-jot) does not run this bundled cleanup pass — it has its own separate per-run "Clean Up Transcript" toggle described below. The original raw transcript is always preserved in the library so nothing is ever lost. If the cleanup model fails to load or inference fails for any reason, the surface silently falls back to the raw transcript — cleanup is treated as an enhancement, not a gate. Very short utterances (a few words) skip cleanup entirely so single-word answers like "yes" or "tomorrow" aren't ever shortened away. This pass is distinct from and independent of the optional [downloaded rewrite model](#7-2-on-device-ai-rewrite-model); the two can be on at the same time, in which case the rewrite model operates on the already-cleaned text. The legacy "Clean Up Transcript" parameter on the ["Transcribe Audio with Jot" Shortcuts action](#10-1-shortcuts-transcribe-audio-with-jot) is a separate per-run flag that uses Apple Intelligence and continues to default to off.

### 7.2 On-Device AI Rewrite Model
Jot offers an optional on-device language model download that enables a full prose rewrite of any transcript. The model runs entirely on-device with no data sent to any server. Fresh installs default to a recommended on-device model (about 2.5 GB on disk); the [Switch Model Picker](#7-9-switch-model-picker) lets users select an alternate model instead, and existing users who already have a previous model installed are not silently flipped to the new default. The automatic model download is triggered in two ways: enabling the [AI Rewrite master toggle](#7-3-ai-rewrite-master-toggle) while the model is not installed, or opening Settings → AI Rewrite when the toggle is already on and the model is not yet installed. Users can also initiate a download from a third entry point: tapping the Transform action in the [Transcript Detail view](#3-transcript-detail) when the model is not ready surfaces the [Download Pitch sheet](#7-10-download-pitch-sheet), which contains a one-tap "Download · 2.5 GB" CTA that starts the download. This detail-flow entry point requires an explicit tap on the pitch sheet CTA — it is not automatic. A fourth entry point is the [wizard's optional download step](#4-10-ai-rewrite-download-offer-optional-step-2). Every download CTA and model strip displays the currently-selected model's name and exact size, so the user sees what they are downloading before they tap.
...
Users can maintain a list of domain-specific words, names, or technical terms that Jot should recognize correctly during transcription. The list is managed from [Settings → Vocabulary](#6-2-vocabulary-settings-link) and is also seeded during the [optional wizard step](#4-9-vocabulary-seed-optional-step-1).
...
A Contact section at the bottom of the Help screen provides a button that opens the iOS Mail app with a pre-populated recipient address. Tapping it leaves Jot and hands off to Mail.
...
The [Setup Wizard](#4-3-keyboard-installation--full-access-w3) explains why Full Access is required for the keyboard: it allows the keyboard to access the transcript result produced by the main app in order to paste it into the host app. The [Help screen](#9-4-privacy-explainer) does not explain the Full Access rationale — it mentions Full Access only in a troubleshooting entry directing users to enable it when paste does not work. The [Settings → Privacy](#6-4-privacy-controls) section includes a Full Access informational row. The Full Access toggle status shown in Settings is a static label — Jot cannot read the system's current Full Access setting and does not update this label dynamically. After setup, if the user is using the keyboard and Full Access has been disabled or was never granted, the keyboard's locked-state pill bounces them into the [Full Access Prompt Screen](#5-14-full-access-prompt-screen) — the runtime equivalent of the wizard's W3 disclosure.
