...
- [2. Recording Experience](#2-recording-experience)
  ...
  - [2.14 Model-Loading Progress](#2-14-model-loading-progress)
...
While the user speaks, a scrolling text area displays a continuously updating live preview of what is being said, rendered in an italic serif font. The view auto-scrolls to follow the newest text so the user always sees the most recent words without manually scrolling. A soft fade at the top edge signals that more content exists above. The live preview can be turned off entirely in [Settings → Live text while dictating](#6-1-1-live-text-while-dictating) — recording then shows the waveform and timer, and the transcript appears on stop.
...
### 2.14 Model-Loading Progress
When a dictation starts while the [speech model](#6-1-speech-model-management) is still loading — typically the first dictation after an install, an update, or the system having reclaimed memory — Jot shows a determinate progress bar instead of an open-ended spinner, on both the [recording surface](#2-1-full-screen-recording-surface) and the [keyboard streaming strip](#5-3-streaming-strip-recording-state). It is not a literal byte-count percentage (the model load reports none); instead the bar is paced against how long the model actually took to load on that specific device before, so it reflects real progress and self-tunes over time — and if a cold load runs longer than expected, the bar eases toward the end and waits rather than claiming it has finished. Throughout, recording is already live ([§2.9](#2-9-auto-dismiss-on-completion)): a short "Keep speaking — your words are saved and appear when loading finishes" line tells the user they can talk right away, and the [live transcript](#2-3-live-streaming-transcript) replaces the indicator the moment the model is ready.
...
When a dictation is in progress inside the keyboard, the top strip transforms into a live recording display: a blue pulsing dot, an elapsed timer, a six-bar animated waveform, a brief header status line ("We tidy this up when you stop") noting the live text gets a cleanup pass once recording ends, and a scrollable pane that shows the [streaming partial transcript](#2-3-live-streaming-transcript) in real time, rendered in the same small italic serif used by the recording surface. When a [cold start](#2-9-auto-dismiss-on-completion) begins before the [speech model](#6-1-speech-model-management) has finished loading, the pane first shows a "Loading…" state — the [model-loading progress bar](#2-14-model-loading-progress) plus a "keep speaking, your words are saved" reassurance — in place of the partial text, and switches to the live transcript the moment the model is ready (recording is already capturing the whole time, so nothing said during the load is lost). The pane is bottom-anchored — the first words appear near the bottom of the pane and older lines push upward as more text arrives — with a blinking caret at the trailing edge and a top-edge fade for overflow; the visible stream is capped to a bounded height so it never grows past the strip. While the dictation is [paused](#5-8-pause--resume), the pulsing dot goes static/hollow, the timer freezes, "Paused · mic ready, not capturing" is shown, and the waveform hides; the partial text already shown is kept and resuming appends to it. A "↓ live" pill appears when the user has manually scrolled up and the view is no longer tracking the newest content; tapping the pill restores auto-follow. Follow-up voice commands (see [§2.11](#2-11-chained-follow-up-voice-commands)) apply within the 30-second window after the keyboard dictation completes. If [Live text while dictating](#6-1-1-live-text-while-dictating) is turned off, the strip keeps the dot, timer, and waveform but shows no streaming text — the transcript pastes when you stop, as always.
...
The Settings screen surfaces the name and current status of the installed speech model along with a chip showing its readiness. A chevron opens a sub-screen where the user can switch between two model variants — "Parakeet 110M (lighter, faster)" and "Parakeet 600M (more accurate)". The lighter "Parakeet 110M" variant is the default and ships bundled with the app — it is always available immediately on install with no download. "Parakeet 600M" is an opt-in download (~440 MB) for users who want higher accuracy. When the bundled 110M variant is selected, no download/re-download button is shown at all — its weights live in the read-only app bundle, so there is nothing to fetch or purge. When the 600M variant is selected and not yet downloaded, the button reads "Download"; once downloaded, the button reads "Re-download", and tapping it shows a "Re-download?" confirmation dialog with a destructive "Re-download" button and a "Cancel" button before the download is initiated. The first-time "Download" does not show this confirmation. The bundled 110M variant cannot be deleted — it lives inside the app bundle and is restored on reinstall. [Vocabulary Boost](#8-vocabulary-boost) applies to either variant. Below the variant row, a "Live text while dictating" switch controls whether words appear on screen as you speak — see [§6.1.1](#6-1-1-live-text-while-dictating).

### 6.1.1 Live Text While Dictating
A "Live text while dictating" switch in the Speech section controls whether words appear on screen as you speak during a [recording](#2-3-live-streaming-transcript) (and on the [keyboard streaming strip](#5-3-streaming-strip-recording-state)). Turning it off saves battery: while recording, the screen shows the waveform and timer instead of live words, and the full transcript appears when you stop — the saved transcript is identical either way, because the final accurate pass always runs on stop. The default follows the device's capability (newer iPhones default on); an explicit user choice is remembered and never changed by app updates. Changes take effect on the next dictation. The [Ask Jot](#14-ask-jot) microphone is unaffected — its live text is how the question is entered.
...
A row in the AI section shows the current status of the [AI Rewrite model](#7-ai-rewrite) and navigates to the full AI Rewrite settings screen where the model can be downloaded, freed, and prompts can be managed. The choice of which model answers [Ask Jot](#14-ask-jot) lives directly in the **AI section of the main Settings screen** (not this sub-screen): a "Use on-board Qwen for Ask" toggle selects Apple Intelligence (default, toggle off) or the on-board model (toggle on) — see [§14.5](#14-5-answer-model-choice).
...
The default Parakeet 110M speech bundle ships inside the app, so it cannot fail to download. This failure mode only applies to the opt-in Parakeet 600M variant (see [§6.1 Speech Model Management](#6-1-speech-model-management)): if a download fails after the user taps "Download" in Settings, the model section surfaces a "Retry download" action.
