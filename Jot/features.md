...
The third panel sends users to Keyboard Settings once to add Jot and turn on Full Access. When either signal is missing, the primary call-to-action reads "Open Keyboard Settings"; only when both the keyboard is installed AND Full Access is granted does it change to "Continue." Jot detects keyboard installation via the system's installed-keyboards list and detects Full Access by reading a mirror flag that the keyboard extension writes each time it appears (iOS gives the main app no direct API for this — the mirror is necessary). One consequence: the Full Access signal stays "not granted" from the main app's perspective until the user has presented the Jot keyboard at least once after enabling it. An always-visible "I've already done this" secondary action advances immediately as an escape hatch. See [§13.3](#13-3-full-access-disclosure).
...
The [Setup Wizard](#4-3-keyboard-installation--full-access-w3) explains why Full Access is required for the keyboard: it allows the keyboard to access the transcript result produced by the main app in order to paste it into the host app. The [Help screen](#9-4-privacy-explainer) does not explain the Full Access rationale — it mentions Full Access only in a troubleshooting entry directing users to enable it when paste does not work. The [Settings → Privacy](#6-4-privacy-controls) section includes a Full Access informational row whose status chip is a static label — iOS does not expose Full Access state to the main app directly, and Settings does not update this label dynamically. The wizard's W3 step works around the limitation by reading a flag that the keyboard extension writes to the App Group on each presentation, but the signal is only available once the user has presented the keyboard at least once after enabling Full Access. After setup, if the user is using the keyboard and Full Access has been disabled or was never granted, the keyboard's locked-state pill bounces them into the [Full Access Prompt Screen](#5-14-full-access-prompt-screen) — the runtime equivalent of the wizard's W3 disclosure.
