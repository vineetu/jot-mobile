...
- [3. Transcript Detail](#3-transcript-detail)
  ...
  - [3.9 Translate](#3-9-translate)
  - [3.10 Find & Replace (while editing)](#3-10-find--replace-while-editing)
...
- [7. AI Rewrite](#7-ai-rewrite)
  ...
  - [7.10 Setup Routing from Rewrite](#7-10-setup-routing-from-rewrite)
...
### 3.3 Selectable Text
Both the original transcript text and the rewrite text are fully selectable, allowing users to copy any portion to the clipboard using the standard iOS text selection handles. On devices with Apple Intelligence, selecting the text also surfaces the system **Writing Tools** (Rewrite, Make Concise, Proofread, and more) — which Jot points users to when its own [rewrite model isn't downloaded](#7-10-setup-routing-from-rewrite).
...
### 3.7 Edit Transcript
The Edit pencil in the [action bar](#3-5-action-bar) turns the currently-visible tab into an editable text field with a slim bottom bar offering Cancel and a blue Save (✓) button. The tab pill is hidden while editing — one tab at a time. Editing the **Original** tab overwrites the transcript text in place; the pre-edit speech-model output is not retained (the user's correction becomes the new ground truth). Editing the **Rewrite** tab keeps the model's rewrite frozen as the "before" and saves the user's correction as the "after," so the pair is preserved as a future-improvement training signal (see [§13.5](#13-5-rewrite-edit-training-pairs)). Tapping Rewrite after an edit generates a fresh rewrite and clears the prior user-edit; tapping the Rewrite tab's Discard affordance clears both the model rewrite and any user edit together. The Edit pill is disabled while a rewrite is mid-flight and while the back chevron is also disabled mid-edit so accidental swipes don't silently lose work. While editing, a microphone button in the bottom bar lets the user dictate straight into the field: speaking inserts text at the cursor (or replaces the current selection), the words stream in live as they are spoken, and tapping the button again (now a Stop control) finishes and drops the dictated text in place. This in-field dictation pastes into what the user is editing and saves no separate transcript of its own; Cancel and Save stay disabled until the dictation is stopped so the streamed text can't be cut off mid-word. While editing, any text the user **adds or changes during this session** — whether typed or dictated — is shown in *italic*, while the untouched original text stays upright, so it's easy to see exactly what was changed. The italic is a live editing cue only: on Save everything is stored as plain text and the next time the entry is opened it all reads upright again. Edit mode also offers [Find & Replace](#3-10-find--replace-while-editing) for fixing a repeatedly-misheard word in one pass.

### 3.9 Translate
The [Rewrite Prompt Picker](#7-5-prompt-picker) includes a **Translate** option that opens a lightweight translate sheet. The user picks a target language from a row of chips (French, Spanish, German, Italian, Portuguese, and more) and the active tab’s text — whether the [Original or the Rewrite](#3-1-original--rewrite-tabs) — is translated instantly, shown as a quote with **Copy** and **Share** actions. Translation is **read-once**: it is not saved to the transcript, does not occupy the Rewrite tab, and leaves both the original and any rewrite untouched. It runs **entirely on this iPhone** using the system’s built-in translation (offline after a one-time per-language download the system manages), so the text never leaves the device ([§13.1](#13-1-fully-on-device-processing)). It is distinct from the optional “Translate to…” [rewrite-prompt template](#7-12-new-prompt-sheet), which runs through the on-device rewrite model and *saves* into the Rewrite tab — this Translate option is faster, needs no model download, and is ephemeral.

### 3.10 Find & Replace (while editing)
While [editing a transcript](#3-7-edit-transcript), a magnifying-glass button in the edit bar opens a **Find & Replace** bar above the keyboard — for when the speech model misheard the same word several times. The user types a word to find (whole-word, case-insensitive matches are counted live) and a replacement, then taps **Replace All** to fix every occurrence at once; the changed words show in *italic* like any other edit until Save. Find & Replace operates on whichever tab is being edited (Original or Rewrite) and is available only in edit mode. When a replacement looks like a genuine term correction — a one- or two-word term that isn't an everyday word, swapped in two or more places — Save shows a gentle one-tap offer to **add the corrected term to your [vocabulary](#8-1-custom-term-list)**: it adds the term, remembers the misheard form as a "sounds-like" so the [keyboard's correction prompts](#8-8-vocabulary-correction-prompts-in-the-keyboard) can catch it, and teaches the system so the next dictation gets it right by itself (the same path as selecting a word and choosing Add to Vocabulary). The offer is dismissible and never blocks; declining changes nothing.
...
### 5.6 Actions Pane
The ••• actions button opens a full-width **actions pane** in the keyboard's top region (in place of the [recents strip](#5-2-recents-strip-idle-state), never overlaying the controls), laid out as a grid of tiles. Feature tiles: **Add to Vocabulary** — adds the host's currently-selected word to the [Vocabulary](#8-vocabulary-boost) term list (enabled only with a selection); **AI Rewrite** and **Translate** — short guides (nothing runs in the keyboard) that teach iOS's own selection-menu tools, showing the actual menu glyph to tap rather than just naming it, with the headings set in the app's display face. Edit tiles: **Copy**, **Paste**, and **Undo** / **Redo** — Undo/Redo work off a stack (up to 20 steps) and each shows a **count badge** of how many steps remain that way; tapping Undo moves one step to the redo side and vice-versa, so the badges always reflect what's available. A **Close** tile dismisses the pane (tapping ••• again also closes it). Results of these actions surface in the [status banner](#5-10-status-banner).
...
### 5.10 Status Banner
Outcomes surface as a liquid-glass status banner inside the keyboard, colour-coded by meaning so it reads at a glance: green for a success (e.g. a word added to [Vocabulary](#8-vocabulary-boost) from the [Actions pane](#5-6-actions-pane)), amber for a heads-up ("common word — not added", "select a word first"), a soft red for a genuine error (e.g. a paste that couldn't land — the text is saved to the clipboard to tap instead), and a blue spinner while something is in progress. It keeps the user informed without leaving the host app, appears the instant the action completes, and clears itself after a couple of seconds.
...
A row in the AI section shows the current status of the [AI Rewrite model](#7-ai-rewrite) and navigates to the full AI Rewrite settings screen where the model can be downloaded, freed, and prompts can be managed. The screen opens with a **rewrite-engine choice** — **Apple Intelligence** (the device's built-in [Writing Tools](#3-3-selectable-text); the default when the device supports it, with no download) or **Jot's AI** (the downloadable [Qwen model](#7-2-on-device-ai-rewrite-model)). Choosing Apple Intelligence hides the saved-prompt list and shows a short how-to instead (Writing Tools don't use Jot's prompts); choosing Jot's AI shows the model status and the user's prompts. Switching to Apple Intelligence is also how a user turns the downloaded model off and frees its space (see [§7.10](#7-10-setup-routing-from-rewrite)). There is no longer a choice of which model answers [Ask Jot](#14-ask-jot) — Ask always uses Apple Intelligence, so the former "Use on-board Qwen for Ask" toggle has been removed (see [§14.5](#14-5-answer-model-choice)).
...
If the [AI Rewrite model](#7-2-on-device-ai-rewrite-model) download fails or the model becomes unavailable, [Settings → AI](#7-8-model-download-management) surfaces retry and reload actions. In the Transcript Detail view, tapping Rewrite when the model has not been downloaded (or is mid-download / loading / errored) routes the user to AI Rewrite settings as a sheet — see [§7.10](#7-10-setup-routing-from-rewrite) — where the same retry/download controls live. An error card with a dismiss option appears only after a rewrite attempt actually fails (see also [§12.4](#12-4-rewrite-error)).
