...
While the user speaks, a scrolling text area displays a continuously updating live preview of what is being said, rendered in an italic serif font. The preview is produced by the same accurate model re-reading the most recent stretch of audio — refreshed in sentence-sized chunks at natural pauses and every few seconds — so it is a rough running approximation, not the final text (see [§2.8](#2-8-live-preview--final-transcript)). Each chunk simply appears in full as it arrives (there is no longer a word-by-word fade-in animation), and while the model is still working a small stepping-ellipsis "still transcribing" tail trails the latest words. The view auto-scrolls to follow the newest text so the user always sees the most recent words without manually scrolling. A soft fade at the top edge signals that more content exists above. The live preview can be turned off in [Settings → Dictation](#6-1-speech-model-management) with the "Live text while dictating" toggle (on by default; turning it off saves battery and does not change the saved transcript); on lower-memory iPhones the live preview is off by default for the same reason.
...
Most of the time the [speech model](#6-1-speech-model-management) is ready instantly and a dictation just shows the normal "Listening" state — there is no progress bar, percentage, or spinner. Only when a load is genuinely slow — typically the first dictation after an install, an update, or the system having reclaimed the app's memory — does Jot show a short, reassuring line in place of the live text, on both the [recording surface](#2-1-full-screen-recording-surface) and the [keyboard streaming strip](#5-3-streaming-strip-recording-state). It never shows a number, a bar, or the model's size or name; if the load drags on, the line rotates through a few gently-worded messages so the wait feels attended-to rather than stuck. Throughout, recording is already live and capturing ([§2.9](#2-9-auto-dismiss-on-completion)): nothing the user says during the wait is lost, and the [live transcript](#2-3-live-streaming-transcript) replaces the line the moment the model is ready and catches up on the buffered audio.
...
The fifth panel is a guided hands-on flow built around a sample practice field. Tapping the field raises the keyboard. If the system keyboard comes up instead of Jot, a cue in the field tells the user to tap the globe key to switch to the Jot keyboard, and if they linger the hint escalates to point out it's the bottom-left key. Once the user switches to Jot and dictates, the words stream into the keyboard's [streaming strip](#5-3-streaming-strip-recording-state) and then paste into the practice field as a visible "it works" moment. The inserted result follows the normal [automatic cleanup](#7-1-automatic-cleanup) path. The step does not auto-advance — the user taps Continue themselves once they've seen it work. So the model is ready by this step, Jot begins warming the [speech model](#6-1-speech-model-management) the moment the wizard opens. This is the wizard's only "try it" step — practicing inside the wizard surface was dropped so users experience Jot exactly as they will use it after setup.
...
Jot uses a single on-device speech model, chosen automatically for the device, so there is no model picker and nothing to download or manage. The model ships inside the app, so dictation works instantly and offline the moment Jot is installed; on lower-memory iPhones a smaller English model is fetched on first use instead. Because there is only one model and it is selected automatically, the Settings card for it is now labelled **"Dictation"** and carries just two controls — no model name, readiness pill, variant picker, or "runs on this iPhone" footer:

- **Language** — a native dropdown selector. English is the only choice today; more languages will appear here as they ship.
- **Live text while dictating** — a toggle (on by default) for the [live preview](#2-8-live-preview--final-transcript) that shows words as you speak. Turning it off saves battery and does not change the saved transcript. On lower-memory iPhones it starts off by default.

[Vocabulary Boost](#8-vocabulary-boost) applies to dictation regardless of these settings.
...
After a dictation, when Jot has a candidate correction tied to the user's custom vocabulary — a spot where it suspects a heard word should have been one of the user's saved terms — the [Jot Keyboard](#5-jot-keyboard) surfaces a small card asking the user to confirm or answer it (for example, "what should this say?"), one question at a time, with a Skip option. Answering teaches Jot how that term is heard so future dictations get it right; skipping moves on without changing anything. These prompts and the confirmations the user gives are private and stay on the device — nothing about them leaves the iPhone. The corrections feed the user's [vocabulary](#8-1-custom-term-list) and are independent of the [Vocabulary Boost toggle](#8-6-vocabulary-boost-toggle)'s on/off state for biasing.
...
On most iPhones the [speech model](#6-1-speech-model-management) ships inside the app, so it cannot fail to download. The only download-failure path is on lower-memory iPhones, where a smaller English model is fetched on first use; if that fetch fails the dictation surfaces an error and retries on the next attempt.
...
**iCloud Device Backup behavior.** When the user has iCloud Backup enabled in iOS Settings, the following are included in their device backup automatically: transcripts (Original + Rewrite), saved AI Rewrite prompts, custom vocabulary, and app preferences. Audio is never written to disk so it can't be part of any backup. Downloaded model weights are explicitly kept OUT of the backup so they don't bloat it (typical Jot backup size: a few MB, not GB): the AI Rewrite model (Qwen 3.5 4B, ~2.5 GB) lives in a location iOS unconditionally excludes from backup, and any downloaded speech-model weights are explicitly marked excluded at launch so they don't ride the backup. The bundled speech model needs no backup (it reinstalls with the app); any downloaded weights re-download on first use after a restore.
