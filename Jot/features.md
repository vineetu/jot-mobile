...
### 5.6 Actions Pane
The ••• actions button opens a full-width **actions pane** in the keyboard's top region (in place of the [recents strip](#5-2-recents-strip-idle-state), never overlaying the controls), laid out as a grid of tiles. Feature tiles: **Add to Vocabulary** — adds the host's currently-selected word to the [Vocabulary](#8-vocabulary-boost) term list (enabled only with a selection); **AI Rewrite** and **Translate** — short guides (nothing runs in the keyboard) that teach iOS's own selection-menu tools, showing the actual menu glyph to tap rather than just naming it, with the headings set in the app's serif display face. Edit tiles: **Copy**, **Paste**, and **Undo** / **Redo** — Undo/Redo work off a stack (up to 20 steps) and each shows a **count badge** of how many steps remain that way; tapping Undo moves one step to the redo side and vice-versa, so the badges always reflect what's available. A **Close** tile dismisses the pane (tapping ••• again also closes it). Results of these actions surface in the [status banner](#5-10-status-banner).
