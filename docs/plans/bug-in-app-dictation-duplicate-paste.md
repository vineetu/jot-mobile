...
> **Status: FIX IMPLEMENTED (2026-06-03) — collapsed to one paste path; compiles clean; Codex-reviewed; PENDING on-device verify-once.** Deleted the keyboard's documentIdentifier/keyboardType same-field guards + the in-process `FocusedFieldInsert` side door (file deleted). The keyboard `textDocumentProxy` flush is now the sole deliverer everywhere. **On-device gate before shipping:** in-Jot paste must land EXACTLY once across transcript Edit / Feedback / Wizard W5 (not zero — the old nil-context drop; not twice — the old dup). If it drops, add a nil-context fallback. Original confirmation/diagnosis + fix plan retained below.
